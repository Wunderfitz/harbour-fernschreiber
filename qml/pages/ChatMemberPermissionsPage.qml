...
Dialog {
    ...
    // key: chatPermissions field as TDLib 1.8.67 names it,
    // legacy: older name, still read so data from an older TDLib is understood
    readonly property var permissionItems: [
        ...
        { key: "can_add_link_previews", legacy: "can_add_web_page_previews", label: qsTr("Add Web Page Previews", "member permission") },
    ...
    onAccepted: {
        // Everything chatPermissions carries but this dialog has no switch for --
        // can_create_topics, can_react_to_messages, can_edit_tag, and whatever a
        // later TDLib adds -- is kept as it stands. A field left out of the object
        // counts as false, so rebuilding it from the switches alone would revoke
        // rights the dialog never showed.
        var permissions = { "@type": "chatPermissions" };
        var currentPermissions = (isRestricted ? memberStatus.permissions : defaultPermissions) || {};
        for (var key in currentPermissions) {
            if (key !== "@type") {
                permissions[key] = currentPermissions[key];
            }
        }
